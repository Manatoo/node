...
function Manatoo(apiKey) {
  this.apiKey = apiKey;
  this.event.request = this.request.bind(this);
...
Manatoo.prototype.event = {
  create: function(attrs) {
    if (!attrs || !attrs.name) {
      throw new Error('Event attributes parameter must be defined with a valid name');
    }

    return this.request('post', 'events', humps.decamelizeKeys(attrs))
    .then(function (res) {
      return res.body
    }.bind(this));
  },
